...
VKInstance::VKInstance(std::string applicationName, std::string engineName,
					   Window* pWindow) {
...
void VKInstance::createInstance(std::string applicationName,
	...
	if (enableValidationLayers) {
		createInfo.enabledLayerCount =
			static_cast<uint32_t>(VKPipeline::s_validationLayers.size());
...
void VKInstance::createSurface(Window* pWindow) {
	if (glfwCreateWindowSurface(
			m_instance, const_cast<GLFWwindow*>(pWindow->getGLFWWindow()),
