...
#ifdef __APPLE__
	// kept getting annoying error message on macOS M1 Chip unless this was
	// added!
...
void VKPipeline::init() {
	...
	m_pInstance = std::make_unique<VKInstance>("Vulkan App", "Bird Engine",
											   m_pWindow.get());
	m_pPhysicalDevice = std::make_unique<VKPhysicalDevice>(m_pInstance.get());
	m_pLogicalDevice =
		std::make_unique<VKLogicalDevice>(m_pPhysicalDevice.get());
	m_pSwapChain = std::make_unique<VKSwapChain>(
		m_pPhysicalDevice.get(), m_pLogicalDevice.get(), m_pInstance.get(),
		m_pWindow.get());

	// Start here
	m_pRenderPass = std::make_unique<VKRenderPass>(
		m_pPhysicalDevice.get(), m_pLogicalDevice.get(), m_pSwapChain.get());

	// cleanUpSwapChain() function in SwapChain

	VkPhysicalDeviceProperties pProperties{};
	vkGetPhysicalDeviceProperties(m_pPhysicalDevice->getPhysicalDevice(),
								  &pProperties);

	std::cout << pProperties.limits.maxImageDimension2D << "  "
			  << m_pSwapChain->getSwapChainImageFormat() << std::endl;
}
