...
namespace bird::vk {
    ...
    VKSwapChain::VKSwapChain(const VKPhysicalDevice* physicalDevice, const VKLogicalDevice* device, const VKInstance* instance, const Window* window) : m_pLogicalDevice(device), m_pPhysicalDevice(physicalDevice), m_pWindow(window) {
    ...
    void VKSwapChain::createSwapChain(const VKInstance* instance) {
        SwapChainSupportDetails swapChainSupportDetails = querySwapChainSupport(m_pPhysicalDevice->getPhysicalDevice(), instance->getSurface());

        VkSurfaceFormatKHR surfaceFormat = chooseSwapChainFormat(swapChainSupportDetails.formats);
        VkPresentModeKHR presentMode = chooseSwapPresentMode(swapChainSupportDetails.presentModes);
        VkExtent2D extent = chooseSwapExtent(swapChainSupportDetails.capabilities, m_pWindow);
    ...
    void VKSwapChain::cleanUpSwapChain() {
        ...
        vkDestroyImageView(m_pLogicalDevice->getDevice(), m_colorImageView, nullptr);
        ...
        vkDestroyImageView(m_pLogicalDevice->getDevice(), m_depthImageView, nullptr);
        ...
        for(auto framebuffer : m_swapChainFramebuffers) {
            vkDestroyFramebuffer(m_pLogicalDevice->getDevice(), framebuffer, nullptr);
        }
    ...
    void VKSwapChain::createRenderPass() {
        ...
        if(vkCreateRenderPass(m_pLogicalDevice->getDevice(), &renderPassInfo, nullptr, &m_renderPass) != VK_SUCCESS) {
    ...
    void VKSwapChain::createFramebuffers() {
        ...
        for(int i = 0; i < m_swapChainImageViews.size(); i++) {
            ...
            if(vkCreateFramebuffer(m_pLogicalDevice->getDevice(), &framebufferCreateInfo, nullptr, &m_swapChainFramebuffers[i]) != VK_SUCCESS) {
    ...
    void VKSwapChain::createColorResources() {
        ...
        createImage(m_swapChainExtent.width, m_swapChainExtent.height, 1,
                    VKPipeline::s_msaaSamples, colorFormat,
    ...
    void VKSwapChain::createDepthResources() {
        ...
        createImage(m_swapChainExtent.width, m_swapChainExtent.height, 1, VKPipeline::s_msaaSamples, depthFormat,
                    VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                    m_depthImage, m_depthImageMemory);
        m_depthImageView = createImageView(m_depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
    }

    void VKSwapChain::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits samples, VkFormat format, VkImageTiling tiling,
        ...
        if(vkCreateImage(m_pLogicalDevice->getDevice(), &imageInfo, nullptr, &image) != VK_SUCCESS) {
            throw std::runtime_error("Failed to create image texture!");
        }

        VkMemoryRequirements memoryRequirements;
        vkGetImageMemoryRequirements(m_pLogicalDevice->getDevice(), image, &memoryRequirements);
        ...
        if(vkAllocateMemory(m_pLogicalDevice->getDevice(), &allocateInfo, nullptr, &imageMemory) != VK_SUCCESS) {
    ...
    uint32_t VKSwapChain::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
        VkPhysicalDeviceMemoryProperties memoryProperties;
        vkGetPhysicalDeviceMemoryProperties(m_pPhysicalDevice->getPhysicalDevice(), &memoryProperties);
    ...
    VkFormat VKSwapChain::findSupportedFormat(const std::vector<VkFormat> &candidates, VkImageTiling tiling,
                                                VkFormatFeatureFlags features) {
        for(auto format : candidates) {
            VkFormatProperties props;
            vkGetPhysicalDeviceFormatProperties(m_pPhysicalDevice->getPhysicalDevice(), format, &props);
