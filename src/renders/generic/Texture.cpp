...
TextureBuilder::TextureBuilder(void* pData, uint32_t width, uint32_t height,
...
std::shared_ptr<Texture> TextureBuilder::build() {
	if (m_generateMipMaps) {  // TODO: actually finish this function with mipmap
							  // generation and all
	}
	...
	if (CURRENT_GRAPHICS_PIPELINE == GRAPHICS_PIPELINE_OPENGL) {
		...
		tex = std::make_unique<gl::GLTexture>(m_pData, m_width, m_height,
											  m_format, texID);
		glBindTexture(GL_TEXTURE_2D, 0);
	} else {
		throw std::runtime_error(
			"Graphics pipeline not supported for building images!");
	}

	return tex;
}

Texture::Texture(void* pData, uint32_t width, uint32_t height,
				 TextureFormat format)
	: m_pData(pData), m_width(width), m_height(height), m_format(format) {}
...
const TextureFormat& Texture::getFormat() {
	return m_format;
}
...
TextureBuilder Texture::create(const std::string& path) {
	...
	uint8_t* pixels = stbi_load("../textures/texture.jpg", &width, &height,
...
TextureBuilder Texture::create(void* data, uint32_t width, uint32_t height,
							   TextureFormat type) {
